...
class GoogleHome {
    ...
    detectColorTemperatureUnit(id) {
        this.adapter.getForeignObject(id, (error, object) => {
            // detect color temperature unit -> quick & dirty!
            // see: https://en.wikipedia.org/wiki/Mired
            // mired: when value between 0 and 1000
            // kelvin: value between 1000 and infinity

            let temperature_unit = 'unknown';

            if (!error && object) {
                if (object.common && object.common.min && object.common.max) {
                    if (object.common.min > 0 && object.common.max < 1000) {
                        temperature_unit = 'mired';
                    } else if (object.common.min > 1000) {
                        temperature_unit = 'kelvin';
                    }
                }
            }
            // log message when no unit was found
            if (temperature_unit === 'unknown') {
                this.adapter.log.warn('[GHOME] Could not detect color temperature unit of device: ' + id);
            }

            return temperature_unit;
        });       
    }
    detectScaling(id, min, max) {
        this.adapter.getForeignObject(id, (error, object) => {
            // detect scaling factor -> quick & dirty!

            let scaling = 1;

            if (!error && object) {
                if (object.common && object.common.min && object.common.max) {
                    if (object.common.min != min && object.common.max != max) {
                        scaling = (object.common.max - object.common.min) / (max - min);
                    }
                } else {
                    this.adapter.log.error('[GHOME] No min and max values found for calculating the scaling factor on object: ' + id);
                }
            } else {
                this.adapter.log.error('[GHOME] Error detecting the scaling for object: ' + id);
            }
        });
    }
        ...
        try {
            ...
            if (typeof smartName === 'object' && smartName && smartName.ghType && objects[id].type !== "channel" && objects[id].type !== "device") {
                ...
                try {
                    ...
                    this.adapter.log.error(JSON.stringify(controls));
                    // if (id.indexOf("7f7") !== -1)
                    //     debugger
                    if (controls) {
